...
> **Note on model files.** The serialised model artefacts (`models/tpot_best_pipeline.pkl`
> and `models/scaler.pkl`) are not yet present in this repository. Until they are added,
> generate them locally in one step:
>
> ```bash
> python scripts/run_full_pipeline.py
> ```
>
> This writes both files into `models/`, after which every example below runs as written.
...
│
├── models/                    # Trained models (generated by the pipeline)
│   ├── tpot_best_pipeline.pkl # Trained TPOT model (binary, generated)
│   ├── scaler.pkl            # Data imputer/scaler (binary, generated)
│   ├── tpot_best_pipeline_reference.py # Human-readable pipeline definition
...
*Requires `models/tpot_best_pipeline.pkl`. See the note at the top of this README if it is
not present — `python scripts/run_full_pipeline.py` generates it.*
